...
describe('POST /fragments', () => {
  ...
  test('creates a text/plain fragment', async () => {
    ...
    // Check all fragment properties are included
    ...
    expect(res.body.fragment.created).toBe('2025-10-05T10:00:00Z');
    expect(res.body.fragment.updated).toBe('2025-10-05T10:00:00Z');
    
    // Check Location header with full URL
    expect(res.header.location).toBeDefined();
    expect(res.header.location).toMatch(/\/fragments\/abc123$/);

    // Verify fragment methods were called
